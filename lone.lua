...
makeLabel(macroCard, "Q: 2 + click + 1", 22, 190, 12, false)
makeLabel(macroCard, "E: 1 + click + 1", 163, 190, 12, false)
...
UIS.InputBegan:Connect(function(input, processed)
	...
	if macroEnabled and bindMatches(macroQBind, input.KeyCode) then
		runMacroQ()
		return
	end
	if macroEnabled and bindMatches(macroEBind, input.KeyCode) then
		runMacroE()
		return
	end
	...
	if bindMatches(focusBind, input.KeyCode) then
		focusEnabled = not focusEnabled
		focusButton.Text = focusEnabled and "Enabled" or "Disabled"
		notify(focusEnabled and "shitft enabled" or "Shitft disabled")
		return
	end
end)
...
local function startupLoad(seconds)
	...
	for i = seconds, 1, -1 do
		title.Text = "loading lone v2 admin panel in " .. tostring(i)
		glow1.Text = title.Text
		task.wait(1)
	end
	floatTween:Cancel()
